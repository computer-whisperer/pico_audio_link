...
use embassy_usb::class::uac1::{FeedbackRefresh, SampleWidth};
...
use crate::terminal_type::TerminalType;

const MAX_AUDIO_CHANNEL_COUNT: usize = 1;
const MAX_AUDIO_CHANNEL_INDEX: usize = 0;
...
impl Channel {
    ...
    fn get_channel_config(&self) -> ChannelConfig {
        match self {
            Channel::OnlyChannel => ChannelConfig::OnlyChannel,
        }
    }
}
...
impl<'d, D: Driver<'d>> Microphone<'d, D> {
    ...
    pub fn new(
        ...
        for sample_rate_hz in sample_rates_hz {
            ...
            format_descriptor.push(((sample_rate_hz >> 8) & 0xFFu32) as u8).unwrap();
            format_descriptor.push(((sample_rate_hz >> 16) & 0xFFu32) as u8).unwrap();
        }
        ...
        alt.endpoint_descriptor(
            ...
            &[
                0x00,                                 // bRefresh (0)
                0x00,                                   // bSynchAddress (the feedback endpoint)
            ],
        );
...
impl<'d> Control<'d> {
    ...
    fn interface_set_request(&mut self, req: control::Request, data: &[u8]) -> Option<OutResponse> {
        ...
        if interface_number != <InterfaceNumber as Into<u8>>::into(self.control_interface_number) {
